#!/bin/bash
# Stage2 LoRA fine-tune on LiDAR-LLM-Nu-Grounding (3dtesting).
# - Reuses the stage1 mm_projector (frozen).
# - Loads single-frame LiDAR features keyed by sample_token from lidarclip/stage1_features.
# - Output: ../mllm/checkpoints/vtimellm-<MODEL_VERSION>-3dtesting

MODEL_VERSION=vicuna-v1-5-7b
gpu_vis=3
MASTER_PORT=29577

IFS=',' read -r -a GPU_LIST <<< "$gpu_vis"
NUM_GPUS=${#GPU_LIST[@]}

CUDA_VISIBLE_DEVICES=$gpu_vis torchrun --nproc_per_node=$NUM_GPUS --master_port $MASTER_PORT vtimellm/train/train.py \
    --lora_enable True \
    --model_name_or_path ../base_model/vicuna-v1-5-7b \
    --version v1 \
    --data_path ../3dtesting_dataset/train.json \
    --feat_folder ../lidarclip/stage1_features \
    --pretrain_mm_mlp_adapter ../mllm/checkpoints/vtimellm-$MODEL_VERSION-stage1/mm_projector.bin \
    --output_dir ../mllm/checkpoints/vtimellm-$MODEL_VERSION-3dtesting \
    --bf16 True \
    --num_train_epochs 3 \
    --per_device_train_batch_size 8 \
    --gradient_accumulation_steps 4 \
    --evaluation_strategy "no" \
    --save_strategy "epoch" \
    --save_total_limit 4 \
    --learning_rate 1e-4 \
    --freeze_mm_mlp_adapter True \
    --lora_r 128 \
    --lora_alpha 256 \
    --weight_decay 0.01 \
    --warmup_ratio 0.1 \
    --lr_scheduler_type "cosine" \
    --logging_steps 1 \
    --tf32 True \
    --model_max_length 2048 \
    --gradient_checkpointing False \
    --dataloader_num_workers 4 \
    --lazy_preprocess True \
    --report_to wandb \
    "$@"
